...
namespace ObvpSolver {
    ...
    void Plan_S3_EP(const Eigen::Matrix<double, 3, Eigen::Dynamic>& start_state,
        ...
        eigen_matrix(0, 7) = 1120.0 * (33.0 * a0.transpose() * a0 + 8.0 * q0.transpose() * v0 - 8.0 * qe.transpose() * v0)[0] / w_t;
        eigen_matrix(0, 8) = 8.0 * (29760.0 * a0.transpose() * v0 - 9261.0 * w_t + 1000.0 * q0.transpose() * q0 - 2000.0 * q0.transpose() * qe + 1000.0 * qe.transpose() * qe) / w_t;
    ...
    void Plan_S3_EPV(const Eigen::Matrix<double, 3, Eigen::Dynamic>& start_state,
        const Eigen::Matrix<double, 2, Eigen::Dynamic>& end_state,
        const double w_t,
        double& T,
        Eigen::Matrix<double, 6, Eigen::Dynamic>& C) {
        Eigen::VectorXd q0 = start_state.row(0);
        ...
        Eigen::VectorXd alpha = (-40.0 * pow(T, 2) * a0 - 200.0 * T * v0 - 120.0 * T * ve - 320.0 * q0 + 320.0 * qe) / std::pow(T, 5);
        Eigen::VectorXd beta = 4.0 * (7.0 * pow(T, 2) * a0 + 32.0 * T * v0 + 18.0 * T * ve + 50.0 * q0 - 50.0 * qe) / std::pow(T, 4);
        Eigen::VectorXd gamma = (-8.0 * pow(T, 2) * a0 - 28.0 * T * v0 - 12.0 * T * ve - 40.0 * q0 + 40.0 * qe) / std::pow(T, 3);
